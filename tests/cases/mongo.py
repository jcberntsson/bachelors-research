...
class Mongo(Base):
    ...
    def follow(self):
        ...
        def run(inner_self):
            self.db["races"].update(
                {"_id": inner_self.race_id, "activities": {"$elemMatch": {"participating": inner_self.participant_id}}},
                {"$push": {"activities.$.following": inner_self.follower_id}})

        def teardown(inner_self):
            self.db["races"].update(
                {"_id": inner_self.race_id, "activities": {"$elemMatch": {"participating": inner_self.participant_id}}},
                {"$pull": {"activities.$.following": inner_self.follower_id}})
    ...
    def insertCoords(self):
        ...
        def run(inner_self):
            for coord in inner_self.coords:
                self.db["races"].update({"_id": inner_self.race_id,
                                         "activities": {"$elemMatch": {"participating": inner_self.participant_id}}},
                                        {"$push": {"activities.$.coordinates": coord}})

        def teardown(inner_self):
            for coord in inner_self.coords:
                self.db["races"].update({"_id": inner_self.race_id,
                                         "activities": {"$elemMatch": {"participating": inner_self.participant_id}}},
                                        {"$pull": {"activities.$.coordinates": coord}})
    ...
    def unparticipate(self):
        ...
        def run(inner_self):
            self.db["races"].update({"_id": inner_self.race_id},
                                    {"$pull": {"activities": {"participating": inner_self.participant_id}}})
    ...
    def fetchCoords(self):
        ...
        def run(inner_self):
            race = self.db["races"].find_one({"_id": inner_self.race_id}, {"coordinates": 0, "activities": {
                "$elemMatch": {"participating": inner_self.participant_id}}, "activities.participating": 0,
                                                                           "activities.following": 0,
                                                                           "activities.joinedAt": 0})
    ...
    def removeCoords(self):
        ...
        def run(inner_self):
            self.db["races"].update({"_id": inner_self.race_id},
                                    {"$pull": {"coordinates": {"$in": inner_self.coordinates}}})

        def teardown(inner_self):
            self.db["races"].update({"_id": inner_self.race_id},
                                    {"$push": {"coordinates": {"$each": inner_self.coordinates}}})
    ...
    def fetchHotRaces(self):
        ...
        def run(inner_self):
            race = self.db.races.aggregate([{"$unwind": '$activities'}, {
                "$group": {"_id": '$_id', "totalsize": {"$sum": {"$size": '$activities.following'}}}}, {"$limit": 10}])
