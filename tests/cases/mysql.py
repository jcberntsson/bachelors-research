...
class MySQL(Base):
    ...
    def fetchRace(self):
        ...
        def run(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute("SELECT * FROM race INNER JOIN event ON race.event_id=event.id INNER JOIN racemap on racemap.race = race.id WHERE race.ID="+inner_self.race_id)
            result = cursor.fetchall()
            print(result)
            cursor.close()
