...
class MySQL(Base):
    ...
    def initRaceOne(self):
        ...
        for ddl in self.RACEONE_TABLES:
            try:
        ...
        for x in range(self.quantity_of("users")):
            ...
            cursor.execute(
                "INSERT INTO organizer (username,fullname,password,email) "
                "VALUES('%s','Tester','SuperHash','test@mail.com')" % username
            )
            organizers.append(cursor.lastrowid)
        participants = []
        for x in range(self.quantity_of("organizers")):
            participant_name = "participant_" + str(x)
            cursor.execute(
                "INSERT INTO participant (username,fullname,password) "
        ...
        for x in range(self.quantity_of("events")):
            ...
            for y in range(self.quantity_of("races")):
                ...
                cursor.execute("INSERT INTO map (name) VALUES('%s')" % mapname)
                map_id = cursor.lastrowid
                for p in range(self.quantity_of("race_coordinates")):
                    cursor.execute(
                        "INSERT INTO point (lat,lng,alt,map,orderIndex) "
                        "VALUES(%d, %d, %d, %d, %d)" % (10 + p, 11 + p, 20 + p, map_id, p)
                    )
                cursor.execute(
                ...
                cursor.execute(
                    "INSERT INTO racemap (map,race) "
                    "VALUES('%d','%d')" % (map_id, race_id)
                )
                racemap_id = cursor.lastrowid
                rands = []
                for z in range(self.quantity_of("activities")):
                    # Participants
                    rand = self.new_rand_int(rands, 0, len(participants) - 2)
                    cursor.execute(
                        "INSERT INTO activity (participant,race,joinedAt) "
                        "VALUES('%s','%d','%s')" % (participants[rand], race_id, str(datetime.datetime.now())))
                    activity_id = cursor.lastrowid
                    cursor.execute(
                        "INSERT INTO follow (follower,activity,followedAt) "
                        "VALUES('%d','%d','%s')" % (participants[rand + 1], activity_id, str(datetime.datetime.now()))
                    )
                    for p in range(self.quantity_of("activity_coordinates")):
                        cursor.execute(
    ...
    def commentOnImage(self):
        def setup(inner_self):
            ...
            project_id = cursor.lastrowid
            # Contribution
            cursor.execute("INSERT INTO contribution (contributor,project) VALUES ('" + str(user_id) + "','" + str(
                project_id) + "')")
            contribution_id = cursor.lastrowid
            # Image
            cursor.execute(
                "INSERT INTO image (name,original_name,extension,encoding,size,height,width,verticalDPI,horizontalDPI,bitDepth,createdAt,accepted,project) "
                "VALUES('test_image','original_name','jpg','PNG/SFF',1024,1080,720,40,50,15,'2016-03-03',0,'" + str(
                    project_id) + "')")
            image_id = cursor.lastrowid
            # Output
    ...
    def pairImageSKU(self):
        ...
        def run(inner_self):
            ...
            cursor = self.cnx.cursor()
            cursor.execute(
                "UPDATE image SET sku='" + inner_self.sku_id + "' WHERE project='" + inner_self.project_id + "' AND id='" + inner_self.image_id + "'")
            cursor.close()
            self.cnx.commit()
    ...
    def addRowsToSKU(self):
        def setup(inner_self):
            ...
            inner_self.sku_id = str(sku_id)
            cursor.execute(
                "INSERT INTO header(sku_id,name) VALUES (" + inner_self.sku_id + ",'remove_me1'),(" + inner_self.sku_id + ",'remove_me2'),(" + inner_self.sku_id + ",'remove_me3'),(" + inner_self.sku_id + ",'remove_me4')")
            cursor.close()
        ...
        def run(inner_self):
            ...
            for i in range(10):
                cursor.execute(
                    "INSERT INTO skuValue(sku_id,value,header_name) VALUES (" + inner_self.sku_id + ",'110','remove_me1'),(" + inner_self.sku_id + ",'120','remove_me2'),(" + inner_self.sku_id + ",'130','remove_me3'),(" + inner_self.sku_id + ",'140','remove_me4')")
            self.cnx.commit()
            cursor.close()
    ...
    def follow(self):
        def setup(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute(
                "INSERT INTO participant (username,fullname,password) VALUES('test_follower','Tester','SuperHash')")
            follower_id = cursor.lastrowid
            cursor.execute(
                "INSERT INTO participant (username,fullname,password) VALUES('test_participant','Tester','SuperHash')")
            ...
            cursor.execute(
                "INSERT INTO activity (participant,race,joinedAt) VALUES('" + str(participant_id) + "','" + str(
                    race_id) + "','" + str(datetime.datetime.now()) + "')")
            activity_id = cursor.lastrowid
            cursor.close()
            self.cnx.commit()
        ...
        def run(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute(
                "INSERT INTO follow (follower,activity,followedAt) VALUES ('" + inner_self.follower_id + "','"
    ...
    def insertCoords(self):
        ...
        def run(inner_self):
            ...
            for i in range(100):
                cursor.execute("INSERT INTO activityCoordinate (activity,createdAt,lat,lng,alt) VALUES(" +
                               inner_self.activity_id + ",'" + str(datetime.datetime.now()) + "'," + str(
                    10 + i) + "," + str(11 + i) + "," + str(20 + i) + ")")
            cursor.close()
            self.cnx.commit()
        ...
        def teardown(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute(
                "DELETE FROM activityCoordinate WHERE activity=" + inner_self.activity_id + " AND createdAt > '" + inner_self.start_time + "'")
            cursor.close()
            self.cnx.commit()
    ...
    def fetchParticipants(self):
        ...
        def run(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute(
                "SELECT participant.id, count(*) as followCount FROM participant INNER JOIN activity ON activity.participant=participant.id GROUP BY participant.id ORDER BY followCount")
            result = cursor.fetchall()
            cursor.close()
    ...
    def unparticipate(self):
        def setup(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute(
                "SELECT activity.id, activity.participant,activity.race,activity.joinedAt FROM activity INNER JOIN follow WHERE activity.id=follow.activity")
            result = cursor.fetchall()
            rand = random.randint(0, len(result) - 1)
        ...
        def teardown(inner_self):
            ...
            cursor.execute("INSERT INTO activity (id,participant,race,joinedAt) VALUES('" +
                           str(inner_self.activity[0]) + "','" + str(inner_self.activity[1]) + "','" + str(
                inner_self.activity[2]) + "','" + str(inner_self.activity[3]) + "')")
    ...
    def removeCoords(self):
        ...
        def teardown(inner_self):
            ...
            for p in inner_self.points:
                ...
                map = str(p[4])
                cursor.execute(
                    "INSERT INTO point (id, lat,lng,alt,map) VALUES(" + point_id + "," + lat + "," + lng + "," + alt + "," + map + ")")
            cursor.close()
            self.cnx.commit()
    ...
    def removeRace(self):
        def setup(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute(
                "SELECT id,name,description,race_date,max_duration,preview,location,logo_url,event_id FROM race")
            result = cursor.fetchall()
            rand = random.randint(0, len(result) - 1)
        ...
        def teardown(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute(
                "INSERT INTO race (id,name,description,race_date,max_duration,preview,location,logo_url,event_id) VALUES('" + str(
                    inner_self.race[0]) + "','" + str(inner_self.race[1]) + "','" + str(
                    inner_self.race[2]) + "','" + str(inner_self.race[3]) + "','" + str(
                    inner_self.race[4]) + "','" + str(inner_self.race[5]) + "','" + str(
                    inner_self.race[6]) + "','" + str(inner_self.race[7]) + "','" + str(inner_self.race[8]) + "')")
            for rm in inner_self.racemaps:
                cursor.execute(
                    "INSERT INTO racemap (id,map,race) VALUES('" + str(rm[0]) + "','" + str(rm[1]) + "','" + str(
                        rm[2]) + "')")
            cursor.close()
            self.cnx.commit()
    ...
    def fetchRace(self):
        ...
        def run(inner_self):
            cursor = self.cnx.cursor()
            cursor.execute(
                "SELECT race.*,event.name,racemap.id,racemap.map,p1.lat,p1.lng,p1.alt,p2.lat,p2.lng,p2.alt FROM race INNER JOIN event ON race.event_id=event.id " +
            ...
            cursor.execute(
                "SELECT participant.id, participant.username,participant.fullname FROM activity INNER JOIN participant ON activity.participant=participant.id WHERE race = " + inner_self.race_id)
            participants = cursor.fetchall()
            returnable = dict(race=race, start_point=start_point, goal_point=goal_point, map_coordinates=mapCoords,
